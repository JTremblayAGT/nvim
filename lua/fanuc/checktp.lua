...
function M.setup()
  ...
  end  local function parse_result(result)
    local diag = {}
    
    
    for i, line in ipairs(result) do
      -- Skip empty lines and header information
      if line:match("^%s*$") or line:match("^=+%s*$") or line:match("^%-+%s*$") or line:match("^Analysis Results") or line:match("^File:") or line:match("^Summary") then
        -- Skip these lines
      else
